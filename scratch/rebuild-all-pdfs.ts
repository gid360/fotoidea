...
async function rebuildAllPdfs() {
  const certs = await prisma.certificate.findMany();
  console.log(`Rebuilding PDFs for ${certs.length} certificates...`);
  ...
  try {
    ...
    if (setting && setting.value) {
      template = { ...template, ...JSON.parse(setting.value) };
    }
  } catch {}
  ...
  for (const cert of certs) {
    ...
    const bgColor = hexToRgb(template.bgColor, rgb(0.98, 0.97, 0.96));
    const borderColor = hexToRgb(template.borderColor, rgb(0.84, 0.77, 0.65));
    const textColor = hexToRgb(template.textColor, rgb(0.24, 0.21, 0.18));
    const subtextColor = hexToRgb(template.subtextColor, rgb(0.49, 0.45, 0.40));
    ...
    const titleY = template.titlePosY ? getY(template.titlePosY) : height - 60;
    ...
    centerText(template.titleText || "С Е Р Т И Ф И К А Т", fontBold, 13, titleY - 12, textColor);
    if (template.subtitleText) {
      centerText(template.subtitleText, fontRegular, 6.5, titleY - 22, subtextColor);
    }
    ...
    const codeY = template.codePosY ? getY(template.codePosY) : height - 130;
    ...
    const detailsY = template.detailsPosY ? getY(template.detailsPosY) : height - 165;
    centerText(mainServiceTitle, fontBold, 10, detailsY, textColor);

    let currentY = detailsY - 14;

    const lines = (template.rulesText || "").split("\n").filter(Boolean);
    if (cert.peopleCount) {
    ...
    const qrSectionY = template.rulesPosY ? getY(template.rulesPosY) : height - 295;
    ...
    const phone = template.studioPhone || "+7 777 79 79 888";
    const insta = template.studioInstagram ? `@${template.studioInstagram.replace(/^@/, "")}` : "fotoideakz";
    ...
    const address = template.studioAddress || "г. Уральск, пр. Абулхаир хана 147, ЖК Азимут, 1 этаж";
    centerText(address, fontRegular, 7, 34, subtextColor);

    const pdfBytes = await pdfDoc.save();
    const outPath = path.join(process.cwd(), "scratch", `cert-${cert.code}.pdf`);
    fs.writeFileSync(outPath, pdfBytes);
    console.log(`Rebuilt PDF for cert ${cert.code} (${cert.id}) -> ${outPath} [${pdfBytes.length} bytes]`);
  }

  console.log("All certificate PDFs rebuilt and verified successfully!");
}

rebuildAllPdfs().finally(() => prisma.$disconnect());
